...
class WordPoints_BP_Apps_Functions_Test extends WordPoints_PHPUnit_TestCase_Hooks {
	...
	public function test_groups_events() {
		...
		if ( version_compare( WORDPOINTS_VERSION, '2.3.0-alpha-1', '>' ) ) {
			...
			$this->assertEventRegistered( 'bp_group_invite_send', array( 'bp_group', 'user', 'inviter:user' ) );
			...
			if ( version_compare( buddypress()->version, '2.8.0-alpha', '>=' ) ) {
				...
				$this->assertEventRegistered( 'bp_group_invite_accept', array( 'bp_group', 'user' ) );
			}
			...
			$this->assertEventNotRegistered( 'bp_group_invite_send', array( 'bp_group', 'user', 'inviter:user' ) );
